...
write_csv(output_b25003_wide_state, "b25003_state.csv")
```

The following script pulls data on tenure by age of householder. This data is used
to analyze homeownership rate by age using Table B25007.

```{r Download B25007, echo=T, message=FALSE, warning=FALSE, paged.print=FALSE, results='hide'}
...
# Pull the table variables, excluding Puerto Rico.
b25007_defns <- load_variables(2019, "acs5") %>%
...
# Clean up the output by removing extra text and combining 
output_b25007 <- output_b25007 %>%
  mutate(across(.fns = ~str_remove_all(.x, ", Virginia")),
         across(.fns = ~str_replace_all(.x, "Bedford city", "Bedford County")),
         across(.fns = ~str_replace_all(.x, "51515", "51019")))

# Data needs to be pivoted wider to more easily analyze for homeownership rate.
...
# Write to csv.
write.csv(output_b25007, "b25007.csv")
```
