...
path <- "raw/2007-2023-PIT-Counts-by-CoC.xlsx"

data <- read_excel("raw/2007-2023-PIT-Counts-by-CoC.xlsx") |> 
  clean_names()
...
full_set <- map_dfr(years, function(yr) {
  ...
  if ("co_c_category" %in% names(pit_set)) {
    pit_renamed <- pit_set %>%
      ...
      mutate(year = yr) %>%
      select(co_c_number:co_c_name, co_c_category, overall_homeless, year) %>%
...
write_csv(full_set_va, "data/va_pit.csv")
```
