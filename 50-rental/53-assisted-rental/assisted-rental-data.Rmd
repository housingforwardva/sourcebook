...
# Date updated: 1-16-25
va_subsidies <- read_xlsx("raw/nhpd_subsidies_va_1.16.25.xlsx")
...
va_subsidies_geo <- va_subsidies |> 
  filter(!is.na(longitude)) |> 
  mutate(fulladdress = paste(street_address, city, state, zip_code, sep = ", "))

va_subsidies_na <- va_subsidies |> 
  filter(is.na(longitude)) |> 
  mutate(fulladdress = paste(street_address, city, state, zip_code, sep = ", "))


lat_long <- va_subsidies_na %>%
  geocode(address = fulladdress, method = 'geocodio',
          full_results = TRUE,
          unique_only = FALSE,
          lat = lat, 
...
lat_long_mutate <- lat_long |> 
  mutate(longitude = long,
         latitude = lat)

va_subsidies_geocode <- bind_rows(lat_long_mutate, va_subsidies_geo)

write_csv(va_subsidies_geocode, "data/va_subsidies_geocoded.csv")
