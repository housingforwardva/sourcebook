...
years_full <- 2010:2020
years_partial_1 <- 2010:2014
years_partial_2 <- 2015:2020
...
b25063_defns_2020 <- load_variables(2020, "acs5") %>%
...
b25064_defns <- load_variables(2020, "acs5") %>%
...
b25063_cleaned_2020 <- b25063_defns_2020 %>%
  separate(label, into = c("est", "total", "cash", "rent"), sep = "!!") %>%
  select(variable = name, cash, rent) %>%
  mutate(across(.fns = ~replace_na(.x, "All")),
         across(.fns = ~str_remove_all(.x, ":")))
...
output_b25063_2020 <- map_dfr(years_partial_2, function(yr){
  acs_pull <- get_acs(
    geography = "county",
    table = b25063,
    year = yr,
    ...
    left_join(b25063_cleaned_2020, by = "variable")
...
output_b25063 <- rbind(output_b25063_2014, output_b25063_2020)
```
...
b25064_locality <- output_b25064_locality %>%
  ...
  rename(cpi = mean) %>%
  mutate(dollars20 = ((235.67550/cpi)*estimate))
