...
b25063_defns_2023 <- load_variables(2023, "acs5") %>%
...
b25064_defns <- load_variables(2023, "acs5") %>%
...
b25063_cleaned_2023 <- b25063_defns_2023 %>%
  separate(label, into = c("est", "total", "cash", "rent"), sep = "!!") %>%
  select(variable = name, cash, rent) %>%
  mutate(across(.fns = ~replace_na(.x, "All")),
         across(.fns = ~str_remove_all(.x, ":")))
...
output_b25063_2023 <- map_dfr(years_partial_2, function(yr){
  acs_pull <- get_acs(
    geography = "county",
    table = b25063,
    year = yr,
    ...
    left_join(b25063_cleaned_2023, by = "variable")
...
output_b25063 <- rbind(output_b25063_2014, output_b25063_2023)
```
...
where
$CPI_1$ is the average annual CPI for 2022 and
...
adjustment <- function(x) {
  transform(x, adjusted = ((399.6435/index)*estimate))
}
...
write_csv(output_b25063, "data/b25063.csv")
