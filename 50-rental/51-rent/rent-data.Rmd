...
b25063_defns_2022 <- load_variables(2022, "acs5") %>%
...
b25064_defns <- load_variables(2022, "acs5") %>%
...
b25063_cleaned_2022 <- b25063_defns_2022 %>%
  separate(label, into = c("est", "total", "cash", "rent"), sep = "!!") %>%
  select(variable = name, cash, rent) %>%
  mutate(across(.fns = ~replace_na(.x, "All")),
         across(.fns = ~str_remove_all(.x, ":")))
...
output_b25063 <- rbind(output_b25063_2014, output_b25063_2022)
```
...
adjustment <- function(x) {
  transform(x, adjusted = ((370.2016/index)*estimate))
}
