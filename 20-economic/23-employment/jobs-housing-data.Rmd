...
va_hu <- read_excel("raw/CO-EST2023-HU-51.xlsx", skip = 3) |> 
  janitor::clean_names() |> 
  rename(name_long = x1, base_estimate = x2) |> 
  filter(name_long != "Virginia") |> 
  mutate(across(1, ~str_replace(., ".", ""))) |> 
  mutate(across(1, ~str_remove(., ", Virginia"))) |> 
  drop_na() |> 
  mutate(name_long = str_to_title(name_long))

local_lookup <- read_csv("../../00-admin/va-cbsa-locality-lookup.csv") |> 
  mutate(name_long = str_to_title(name_long))

va_hu_join <- va_hu |> 
  left_join(local_lookup, by = "name_long") |> 
  mutate(fips_full = as.character(fips_full)) |> 
  pivot_longer(2:6,
               names_to = "year",
               values_to = "estimate")|> 
  filter(year != "base_estimate") |> 
  ...
  mutate(fips_full = as.character(fips_full)) |> 
  select(name_long, fips_full, name_short, cbsa_title, estimate, year)
...
# 
# va_hu_localities_2020 <- get_estimates(
#   geography = "county",
#   state = "VA",
#   variables = "H1_001N",
#   year = 2021
...
bls_jobs_pull <- function(jobs_list){
  bls_api(jobs_list,
          startyear = 2010, endyear = 2023, annualaverage = TRUE, Sys.getenv("BLS_KEY")) %>%
    dateCast()
}
...
va_hu_localities_join <- va_hu_localities %>%
  mutate(longdate =
...
va_hu <- rbind(va_hu_localities_join, va_hu_join)


# Combine the the housing unit and labor force data frames together based on year and FIPS.

va_jobs_hu <- va_jobs %>%
  left_join(va_hu, by = c('fips' = 'fips_full', 'year' = 'year'))

va_jobs_hu <- va_jobs_hu %>%
  rename(employment = value, hu = estimate)
```
...
write_csv(va_jobs_hu, "data/va_jobs_housing.csv")
```
