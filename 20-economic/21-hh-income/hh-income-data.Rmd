...
b25119_defns <- load_variables(2023, "acs5") %>%
...
b25119_cleaned <- b25119_defns %>%
  ...
  mutate(across(.fns = ~replace_na(.x, "All")),
         ...
         across(.fns = ~str_remove_all(.x, " in the past 12 months \\(\\in 2023 inflation-adjusted dollars\\)\\ --")))
...
b19049_defns <- load_variables(2023, "acs5") %>%
...
b25118_defns <- load_variables(2023, "acs5") %>%
...
cpi <- fredr(
  series_id = "CUUR0000SEHA" # ID for CPI for All Urban Consumers
) |> 
...
adjustment <- function(x) {
  transform(x, adjusted = ((399.64258/index)*estimate))
}
...
# adjustment <- function(x) {
#   transform(x, dollars22 = ((292.62058/priceindex)*estimate))
# }
# 
# 
